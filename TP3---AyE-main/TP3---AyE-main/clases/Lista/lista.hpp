...
public:
    ...
    Nodo<T> getNodo(int pos);                  // devuelve el nodo de la posicion
    ...
    T *listaToArray(); // convierte la lista en un array de size = size actual
};
...
/**
 * @brief Devuelve el nodo de cierta posicion
 *
 * @tparam T
 * @param  pos - posicion del elemento a tomar
 * @return nodo
...
Nodo<T> Lista<T>::getNodo(int pos)
{
    ...
    return this->resto()->getNodo(pos - 1);
}
